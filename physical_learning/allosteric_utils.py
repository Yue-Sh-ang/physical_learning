...
class Allosteric(Elastic):
	...
	*****************************************************************************************************
	...
	def _mode_plot_2d(self, v, scale, arrows=True, disks=False, figsize=(5, 5), filename=None, ax=None):
		...
		if arrows:
			...
			Zx = ix(X, Y)
			Zy = iy(X, Y)
			ax.streamplot(X, Y, Zx, Zy, color='k', linewidth=1, density=3)

		else:
			# Offset particles directly
		...
		if disks:
			...
			dc = mc.EllipseCollection(
				r, r, np.zeros_like(r),
				offsets=self.pts[self.degree > 0, :self.dim],
				transOffset=ax.transData, units='x',
				edgecolor='k', facecolor=cyclic_cmap(norm(col)),
